          ...
          {packages.map((pkg) => (
            <div
              ...
              className={`relative rounded-2xl p-5 border ${
                pkg.popular
                  ? "bg-gradient-to-br from-[hsl(262,72%,55%)] to-[hsl(270,72%,40%)] text-white border-transparent"
                  : "bg-card border-border"
              }`}
            >
              {pkg.popular && (
                <span className="absolute top-3 right-3 text-[10px] font-bold uppercase tracking-wider bg-white/20 px-2 py-0.5 rounded-full">
                  Best Value
                </span>
              )}
              <div className="flex items-start justify-between mb-3">
                <div>
                  <h3 className={`text-base font-extrabold ${pkg.popular ? "text-white" : "text-foreground"}`}>
                    {pkg.name}
                  </h3>
                  <p className={`text-xs font-medium mt-0.5 ${pkg.popular ? "text-white/60" : "text-muted-foreground"}`}>
                    {pkg.desc}
                  </p>
                </div>
                <div className="text-right">
                  ...
                  <p className={`text-[10px] ${pkg.popular ? "text-white/50" : "text-muted-foreground"}`}>
                    {pkg.perCredit}
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-1.5 mb-4">
                ...
                <span className={`text-sm font-extrabold ${pkg.popular ? "text-white" : "text-foreground"}`}>
