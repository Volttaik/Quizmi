...
export default function FlashcardsPage() {
  ...
  const next = () => { setCurrent((p) => (p + 1) % (activeSet?.cards.length ?? 1)); setFlipped(false); };
    ...
    <div className="min-h-screen bg-background pb-24">
      <div className="max-w-lg mx-auto px-4 pt-6">
        <div className="flex items-center gap-3 mb-6">
          {activeView !== "sets" ? (
            <button onClick={() => setActiveView("sets")} className="w-10 h-10 rounded-full bg-muted flex items-center justify-center text-muted-foreground">
          ...
          <h1 className="text-lg font-extrabold text-foreground flex-1">
            {activeView === "study" ? (activeSet?.title ?? "Study Mode") : activeView === "create" ? "Create Flashcards" : "Flashcard Sets"}
          </h1>
          {activeView === "sets" && (
        ...
        {activeView === "create" ? (
          ...
          <div className="flex flex-col items-center">
            <span className="text-xs text-muted-foreground font-medium mb-4">{current + 1} / {activeSet.cards.length}</span>
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  <div>
                    ...
                    <p className="text-base font-bold leading-relaxed">{flipped ? activeSet.cards[current].back : activeSet.cards[current].front}</p>
                    <p className={`text-[10px] mt-4 ${flipped ? "text-white/30" : "text-muted-foreground"}`}>Tap to flip</p>
                  </div>
                </motion.div>
              </AnimatePresence>
            </div>
            <div className="flex items-center gap-3">
              ...
              <Button variant="outline" size="icon" className="rounded-full w-10 h-10" onClick={() => setFlipped(false)}><RotateCcw className="w-4 h-4" /></Button>
          ...
          <div className="space-y-3">
            {sets.length === 0 ? (
              ...
              <button key={set.id} onClick={() => { setActiveSet(set); setCurrent(0); setFlipped(false); setActiveView("study"); }}
              ...
              >
                ...
                <div className="flex-1"><h3 className="text-sm font-bold text-foreground">{set.title}</h3><p className="text-xs text-muted-foreground font-medium">{set.count} cards</p></div>
